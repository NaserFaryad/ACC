...
void write_ad9833(unsigned int data)
{
    ...
    bcm2835_aux_spi_write(data >> 8);
    bcm2835_aux_spi_write(data & 0xFF);
    bcm2835_gpio_write(RPI_V2_GPIO_P1_12, HIGH);
    // bcm2835_aux_spi_transfern(data >> 8, 1); // high byte first
    // bcm2835_aux_spi_transfern(data & 0xFF, 1); // low byte second
